                                                                        ...
                                                                        const geometry_msgs::WrenchStamped footForce[4])
{
  ...
  for (int legId = 0; legId < footPhase_.size(); ++legId)
  {
    ...
    for (int i = 0; i < kLookaheadSteps; ++i)
    {
      ...
      // NEW: CRITICAL FIX - Move foot BACKWARD during stance
      double currentPhase = fmod(legPhase + (i / (double)swingProfile_.poses.size()), 1.0);
      
      if (currentPhase < 0.5) {  // STANCE PHASE
        // Foot should move BACKWARD relative to body to push body FORWARD
        // Move proportionally to time spent in stance
        double stanceProgress = currentPhase / 0.5;  // 0 to 1
        pose.pose.position.x -= 0.15 * stanceProgress;  // Move backward 15cm total
      } 
